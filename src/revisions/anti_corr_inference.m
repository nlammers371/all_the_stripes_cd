...
%%% Assign detection thresholds to test
cm = jet(128);
sigma = sim_struct(1).noise;
threshold_vec = [0];% sigma 2*sigma 4*sigma 6*sigma];
f_vec_on = [sim_struct(1).fluo_data{:}];
f_vec_off =  [sim_struct(2).fluo_data{:}];
f_bins = linspace(min([f_vec_on f_vec_off]),max([f_vec_on f_vec_off]),500);
f1 = histc(f_vec_on,f_bins);
f1 = f1/sum(f1);
f2 = histc(f_vec_off,f_bins);
f2 = f2/sum(f2);
thresh_fig = figure;
hold on
bar(f_bins,f1,1,'FaceAlpha',.4)
bar(f_bins,f2,1,'FaceAlpha',.4)
for i = 1:numel(threshold_vec)
    plot(repelem(threshold_vec(i),100),linspace(0,1.1*max([f1 f2])),'Color',cm(30,:)/1.2/i,'LineWidth',1.5)
end    
legend('uncorrelated traces','correlated traces')
title('Simulated Detection Thresholds')
ylim([0 1.1*max([f1 f2])]);
xlabel('spot fluorescence (AU)')
ylabel('share')
saveas(thresh_fig,[FigPath 'detection_limits.png'])
...
for thresh = 1:numel(threshold_vec)
        ...
        for t = 1:length(inference_times)
            ...
            for b = 1:n_bootstraps% multiple versions of each replicate                
                ...
                fName_sub = ['eveSet_w' num2str(w) '_K' num2str(K) ...
                    '_' sim_name 'boot_' num2str(b) '_thresh' num2str(thresh)...
                ...
                for f = boot_indices
                    ...
                    f_trace(f_trace<threshold_vec(thresh)) = 0; % apply threshold
    ...
    %                 output.local_runs = local_struct;            
                ...
                output.alpha = alpha;
                output.deltaT = dT;        
                output.detection_threshold = threshold_vec(thresh);
