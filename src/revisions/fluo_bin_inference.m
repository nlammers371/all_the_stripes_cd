...
for f = 1:numel(fluo_bins)    
    for g = 1:length(bin_groups) % loop through different AP groups
        bin = bin_groups(g); % get groups for present iteration                            
        for b = 1:n_bootstrap                
            local_struct = struct;
            init_struct = struct;
            output = struct;        
            ...
            fName_sub = ['eveSet_w' num2str(w) '_K' num2str(K) ...
                '_stripe' num2str(bin) '_fbin' num2str(f) '_t' inference_id];                
            out_file = [out_dir '/' fName_sub];

            % Extract fluo_data        
            stripe_id_vec = round([fluo_inf_struct.stripe_id]);
            fluo_bin_vec = [fluo_inf_struct.FluoBin];
            if bin ~= 0
                trace_filter = ismember(stripe_id_vec,bin)&fluo_bin_vec==f;
            else
                trace_filter = fluo_bin_vec==f;
            end
