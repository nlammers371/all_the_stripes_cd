...
for i = 1:length(cp_filenames) % Loop through filenames    
    ...
    for j = 1:size(traces_clean,2)        
        ...
        edge_frames = (((512-xPos) <= 1.5*snippet_size/2)+(xPos <= 1.5*snippet_size/2)+...
                          ((256-yPos) <= 1.5*snippet_size/2)+(yPos <= 1.5*snippet_size/2))>0;
...
if use_old_boundaries == 1
    ...
    parfor s = 1:length(cp_filenames)
...
for i = 1:length(trace_struct)
    ...
    trace_struct(i).stripe_sub_id = NaN;
...
for i = 1:length(cp_filenames)
    all_ap = [trace_struct([trace_struct.setID]==1).ap_vector];
    all_x = [trace_struct([trace_struct.setID]==1).xPos];
    all_y = [trace_struct([trace_struct.setID]==1).yPos];
...
for i = 1:length(schnitz_struct)
    ...
    s_boundaries = stripe_positions{setID};
    s_id_list = stripe_id_cell{setID};    
    schnitz_struct(i).stripe_id = NaN;
    schnitz_struct(i).stripe_sub_id = NaN;
    schnitz_struct(i).stripe_radius = stripe_radius; 
    classified = 0;
    for j = 1:size(s_boundaries,1)
        sb = s_boundaries(j,:);
        if AP <= sb(5) && AP >= sb(1)
            schnitz_struct(i).stripe_id = s_id_list(j);            
            schnitz_struct(i).stripe_sub_id = 0;
            schnitz_struct(i).stripe_center_ap = sb(3);
            if AP >= sb(4)
                schnitz_struct(i).stripe_sub_id = 1;
            elseif AP <= sb(2)
                schnitz_struct(i).stripe_sub_id = -1;
            end
            classified = 1;
...
save([data_path 'stripe_boundaries_' project '.mat'],'stripe_boundaries')
...
%%
n_boots = 20;
stripe_id_vec = [trace_struct.stripe_id];
stripe_sub_id_vec = [trace_struct.stripe_sub_id];
