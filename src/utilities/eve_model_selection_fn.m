function output_cv = eve_model_selection_fn(fluo_all_ap, AP, n_local, ...
                     n_steps_max, eps, n_cv, K_cv_ls, test_fraction)

% INPUTS:
% fluo_all_ap: structure variable of all traces
% AP: the AP position for which model selection is to be done
%
...
ind_AP = [fluo_all_ap.AP] == AP;

% subset of the structure corresponding to the specified AP
fluo_AP = fluo_all_ap(ind_AP);

% cell variable of traces in the specified AP
fluo_data = cell([1, length(fluo_AP)]);
for i = 1:length(fluo_AP)
    fluo_data{i} = fluo_AP(i).fluo;
end
...
% total number of points in the AP position
n_points_total = 0;
for i = 1:length(fluo_AP)
    n_points_total = n_points_total + length(fluo_AP(i).fluo);
end
...
for ind_cv = 1:n_cv
    ind_full_remaining = 1:length(fluo_AP);
    ind_test_current = [];
    while n_points_test_ind(ind_cv) < n_points_test
        ...
        n_points_test_ind(ind_cv) = n_points_test_ind(ind_cv) + length(fluo_AP(ind).fluo);
        ind_test_current = sort([ind_test_current, ind]);
    end
    test_ind{ind_cv} = ind_test_current;
    train_ind{ind_cv} = setdiff(1:length(fluo_AP), ind_test_current);
end
...
parfor i = 1:n_local_total
    ...
    for j = 1:length(train_ind_local)
        fluo_data_train{j} = fluo_AP(train_ind_local(j)).fluo;
    end
    % testing set
    fluo_data_test = cell([length(test_ind_local), 1]);
    for j = 1:length(test_ind_local)
        fluo_data_test{j} = fluo_AP(test_ind_local(j)).fluo;
    end
