...
%% Plot Dwell Times
med_dwell = zeros(K, length(bin_vec_u));
...
hold on
for k = 1:K
    errorbar(bin_vec_u, med_dwell(k,:),std_dwell(k,:),'LineWidth',1,'Color','black');
end
sctr_dwell = scatter(bin_vec', reshape(dwell_times,1,[])', 75, c_mat,...
    'o', 'filled', 'MarkerFaceAlpha', .2);
sctr_emission_avg = scatter(repelem(bin_vec_u,1,K), reshape(med_dwell,...
...
med_initiation = zeros(K,length(bin_vec_u));
std_initiation = zeros(K,length(bin_vec_u));
med_pi0 = zeros(K,length(bin_vec_u));
std_pi0 = zeros(K,length(bin_vec_u));
med_occupancy = zeros(K,length(bin_vec_u));
std_occupancy = zeros(K,length(bin_vec_u));
med_noise = zeros(1,length(bin_vec_u));
...
hold on
for k = 1:K
    errorbar(bin_vec_u, med_occupancy(k,:),std_occupancy(k,:),'LineWidth',1,'Color','black');
end
plot(reshape(bin_vec_u,1,[])', med_occupancy','black','LineWidth',1);
sctr_occ = scatter(bin_vec', reshape(occupancy_ap,1,[])' , 75, c_mat, 'o','filled', 'MarkerFaceAlpha', .2);
sctr_occ_avg = scatter(repelem(bin_vec_u,1,K), reshape(med_occupancy,1,[]) , 75, repmat(cm,length(bin_vec_u),1), 's','filled', 'MarkerEdgeColor', 'black','MarkerFaceAlpha',1);
box off
...
hold on
for k = 1:K
    errorbar(bin_vec_u, med_initiation(k,:),std_initiation(k,:),'LineWidth',1,'Color','black');
end
sctr_emission = scatter(bin_vec', init_vec , 75, c_mat, 'o','filled', 'MarkerFaceAlpha',.2) ;
sctr_emission_avg = scatter(repelem(bin_vec_u,1,K), reshape(med_initiation,1,[]) , 75, repmat(cm,length(bin_vec_u),1), 's','filled', 'MarkerEdgeColor', 'black','MarkerFaceAlpha',.3) ;
...
med_R_orig = zeros(K,K,length(bin_vec_u));
std_R_orig = zeros(K,K,length(bin_vec_u));
med_A = zeros(K,K,length(bin_vec_u));
std_A = zeros(K,K,length(bin_vec_u));
med_R_fit = zeros(K,K,length(bin_vec_u));
...
for j = 1:K
    ...
    for k = 1:(K-1)
        if plot_orig_rates            
            scatter(bin_vec_u, reshape(med_R_orig(index(k),j,:),1,[]), 50, cmap(increment*((K-1)*(j-1)+k),:), 'o', 'filled', 'MarkerEdgeColor', 'black') ;
        end        
        legend_cell = {legend_cell{:} ['To ' num2str(index(k))]};        
        scatter(bin_vec_u, reshape(med_R_fit(index(k),j,:),1,[]), 50, cmap(increment*((K-1)*(j-1)+k),:), 's', 'filled', 'MarkerEdgeColor', 'black') ;
    end    
    legend(legend_cell{:})
    for k = 1:(K-1)
        if plot_orig_rates
...
for j = 1:K        
    ...
    legend_cell = {};
    for k = 1:(K-1)        
        errorbar(bin_vec_u,reshape(med_R_fit(index(k),j,:),1,[]),reshape(std_R_fit(index(k),j,:),1,[]),'LineWidth',1,'Color','black');
        scatter(bin_index, reshape(R_fit_array(index(k),j,:),1,[]), 75, cmap(increment*((K-1)*(j-1)+k),:), 's', 'filled', 'MarkerFaceAlpha', 0.2) ;                
        scatter(bin_vec_u, reshape(med_R_fit(index(k),j,:),1,[]), 75, cmap(increment*((K-1)*(j-1)+k),:), 's', 'filled', 'MarkerEdgeColor', 'black') ;
    end    
end
xlabel('Relative Position')
ylabel('s^{-1}')
