...
%     set(h,'YTick',1:7)
    s = [];
    for j = 1:7
        e = errorbar(inf_times/60,reshape(r_mat_mean(:,ref_vec==(3*j-(2-i))),1,[]),...
            reshape(r_mat_ste(:,ref_vec==(3*j-(2-i))),1,[]),'Color','black');
        e.CapSize = 0;
        s = [s scatter(inf_times/60,reshape(r_mat_mean(:,ref_vec==(3*j-(2-i))),1,[]),...
            'MarkerFaceColor',cm((j-1)*inc+1,:), 'MarkerEdgeColor', 'black')];
    end
       
    legend(s,ls{:},'Location','southwest')
    xlabel('minutes into nc14')
    ylabel('AU per second')
