...
w = 8;
%State count
ThreeStateFolder = ['D:\Data\Nick\projects\all_the_stripes_cd\fig\experimental_system\eve7stripes_inf\2017-09-08_test\mem8_states3/viterbi_fits/'];
TwoStateFolder = ['D:\Data\Nick\projects\all_the_stripes_cd\fig\experimental_system\eve7stripes_inf\2017-09-08_test\mem8_states2/viterbi_fits/'];
out_dir = 'D:\Data\Nick\projects\all_the_stripes_cd\fig\experimental_system\eve7stripes_inf\2017-09-08_test\mem8_viterbi_analysis\';
    ...
    for i = 1:length(TwoFiles)
        if isempty(strfind(TwoFiles(i).name,['bin' num2str(bin)])) ~= 1
            ...
            for m = 1:length(time_exp)
                minute = 1 + floor(time_exp(m)/60 + .01);
                if minute <= 0
                    continue
                end
                s2 = two_naive_path(m);
...
%         f = histfit(wt_ct,length(wait_bins),'poisson');
    ...
    p3 = plot(1:max_time, o3_normed', 'LineWidth',2);
    legend(p3, 'Off', 'On', 'On2');
    grid on
    title('Occupancy Shares Over Time')
    subplot(2,1,2);    
    o2_normed = occupancy_vec_two ./ repmat(sum(occupancy_vec_two),2,1);
    p2 = plot(1:max_time, o2_normed', 'LineWidth',2);
    legend(p2,'Off', 'On');
    grid on
%     legend('show');
    ...
    v_struct_three(b_ct).binID = bin;
    b_ct = b_ct + 1;
end
save([out_dir 'TwoState_viterbi_dt.mat'],'v_struct_two');
